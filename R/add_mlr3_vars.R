#' Add mlr3 variables to an \code{mldesign_splits} object
#'
#' @param splits (mldesign_splits)
#'
#' @returns (mldesign_splits)
#' @export
...
derive_mlr3_vars <- function(splits){
  ...
  map <- lapply(idx_split_per_idx_train,
                \(x) lapply(x,
                            \(i) data.table(idx_split=i,
